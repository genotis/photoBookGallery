...
- [x] 뷰어: 단일/연속(가로·세로) 모드, 순차 프리로딩(첫 페이지 고정), 키보드 네비게이션, 좌·우 1/3 탭 페이지 넘김, 좌우 스와이프, 전체화면, fit 모드(화면/너비/높이/원본), 읽기 방향(LTR/RTL), 이전/다음 사진집 이동, 슬라이드쇼(인터벌 1~60초), 하단 썸네일 dock(CSS hover 매그니피케이션), UI 자동 숨김 + 중앙 더블탭/마우스 모션 토글, iPad 스와이프 우선 처리. 제목·모델명은 툴바 아래 칩으로 분리(컨트롤과 겹침 방지). (더블페이지·줌은 추후)
...
- [x] 콘텐츠 주소화 이미지 URL — 페이지·표지 URL 에 `?v=<contentHash>` 부착, 응답은 `immutable` 로 최대 캐시. 재압축 시 contentHash 가 바뀌어 URL 도 회전 → 옛 캐시 자동 우회. 서버는 옛 contentHash 키의 디스크 캐시 파일도 즉시 회수.
- [x] 좌측 통합 사이드바 — 내비게이션(전체/랜덤) + 필터(즐겨찾기·경로·모델·출판사·국가·시리즈·태그) + 설정. 패싯 섹션은 `<details>` 로 접기. 우측 별도 패싯바 제거.
- [x] 랜덤 메뉴 — `GET /api/archives/random?n=N`. 좌측 사이드바에서 갯수(설정의 `randomCount`, 5~100) 만큼 랜덤 추출. 같은 메뉴 재클릭 시 시드 회전으로 재섞기.
- [ ] (선택) 7z 지원.
- [ ] (선택) 다중 사용자/권한.
